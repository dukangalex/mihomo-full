...
title "2. 下载并生成安装状态"
mkdir -p "$INSTALL_DIR" "$OUTPUT_DIR" "$INSTALL_DIR/tools"
...
chmod 700 "${INSTALL_DIR}/manage.sh" "${INSTALL_DIR}/generate.sh" "${INSTALL_DIR}/uninstall.sh" "${INSTALL_DIR}/tools/audit-generated-config.sh" "${INSTALL_DIR}/tools/generate-endpoint.py"

# Endpoint 只在首次安装时生成；后续 generate/manage 永远读取 settings.conf，不重新生成。
if [[ -f "${INSTALL_DIR}/settings.conf" ]]; then
  # 已存在的安装状态必须保留固定地址，避免更新时订阅 URL 漂移。
...
# 新安装写入完整状态；已有安装只更新明确允许变化的输入，不改变 Endpoint。
if [[ ! -f "${INSTALL_DIR}/settings.conf" ]]; then
