...
# 所有安装文件必须来自同一个已审核提交，禁止 template/generator/audit 混用不同代码状态。
REPO_COMMIT="d594e552fb32958d1d816c881aea5061b21971b5"
RAW_BASE="https://raw.githubusercontent.com/dukangalex/mihomo-full/${REPO_COMMIT}"
random_path_segment() {
  local v
  if command -v openssl >/dev/null 2>&1; then v=$(openssl rand -hex 10)
  elif [[ -r /dev/urandom ]]; then v=$(od -An -N10 -tx1 /dev/urandom | tr -d ' \n')
  else err "无法安全生成随机订阅路径：需要 openssl 或 /dev/urandom"; fi
...
curl --fail --silent --show-error --location --proto "=https" --tlsv1.2 -o "${INSTALL_DIR}/generate.sh" "${RAW_BASE}/generate.sh" || err "无法下载已审核 generate.sh"
curl --fail --silent --show-error --location --proto "=https" --tlsv1.2 -o "${INSTALL_DIR}/template.yaml" "${RAW_BASE}/template.yaml" || err "无法下载已审核 template.yaml"
curl --fail --silent --show-error --location --proto "=https" --tlsv1.2 -o "${INSTALL_DIR}/manage.sh" "${RAW_BASE}/manage.sh" || err "无法下载已审核 manage.sh"
curl --fail --silent --show-error --location --proto "=https" --tlsv1.2 -o "${INSTALL_DIR}/tools-audit-generated-config.sh" "${RAW_BASE}/tools/audit-generated-config.sh" || err "无法下载已审核最终配置审计脚本"
