...
function main(config) {
  ...
  function getMatchedRegions(proxyName) {
    ...
    const regions = REGIONS.filter(r => {
      try { return new RegExp(r.jsPattern, "i").test(proxyName); } catch (e) { return false; }
    });
  ...
  function normalizeProxyName(proxy) {
    ...
    const matched = getMatchedRegions(originalName);
    const regionFlag = flag || matched.find(r => r.flag)?.flag;
    const normalizedName = regionFlag ? `${regionFlag} ${nameWithoutFlag}` : nameWithoutFlag;
    if (normalizedName !== originalName) regionMatchCache.set(normalizedName, matched);
  ...
  const excludeFilter =
    /群|返利|循环|官网|客服|网站|网址|获取|订阅|流量|到期|机场|下次|版本|官址|备用|过期|已用|联系|邮箱|工单|贩卖|通知|倒卖|防止|国内|地址|频道|无法|说明|使用|提示|访问|支持|教程|关注|更新|作者|加入|超时|收藏|福利|邀请|好友|失联|选择|剩余|公益|发布|DIZTNA|通路|登录|禁止|定时|渠道|牢记|永久|余额|阁下|本站|刷新|导航|建议|重置|以下|防失联|⚠️|@|\bexpire\b|\bhttps?:\/\/|\.com|\btraffic\b/iu;
  ...
  const regionsWithNodes = new Set();
  ...
  for (const raw of filteredRaw) {
    ...
    normalizedProxies.push(finalName === n.name ? n : { ...n, name: finalName });

    const matched = getMatchedRegions(raw.name || "");
    if (matched.length > 0) {
      matched.forEach(r => regionsWithNodes.add(r.name));
    } else {
  ...
  const regionGroups = [];
  const activeRegions = REGIONS.filter(r => regionsWithNodes.has(r.name));
  for (const r of activeRegions) {
    regionGroups.push(...buildRegionTrio(r.name, { filter: r.filter }));
  }
  if (hasOtherRegionNodes) {
    regionGroups.push(...buildRegionTrio(OTHER_REGION_NAME, { "exclude-filter": `(?i)(${allRegionKeywords})` }));
  }

  // 供其他分组引用的"地区选择入口"名字列表：只包含实际生成了分组的地区，
  // 没有节点的地区不会出现在这里，其他分组也就不会引用到不存在的分组名
  const regionNames = [...activeRegions.map(r => r.name), ...(hasOtherRegionNodes ? [OTHER_REGION_NAME] : [])];
  ...
  const domesticGroup = { name: "🇨🇳 国内服务", type: "select", proxies: ["DIRECT", SELECT_NAME, ...regionNames], icon: "" };
  const aiGroup = { name: "🤖 AI服务", type: "select", proxies: [SELECT_NAME, AUTO_NAME, ...regionNamesNoHK], icon: "" };
  const mediaGroup = { name: "📺 Media", type: "select", proxies: [SELECT_NAME, AUTO_NAME, ...regionNames], icon: "" };
  const youtubeGroup = { name: "📺 YouTube", type: "select", proxies: [SELECT_NAME, AUTO_NAME, ...regionNames], icon: "" };
  const googleGroup = { name: "🔍 Google", type: "select", proxies: [SELECT_NAME, AUTO_NAME, ...regionNames], icon: "" };
  const telegramGroup = { name: "📲 Telegram", type: "select", proxies: [SELECT_NAME, AUTO_NAME, ...regionNames], icon: "" };
  const microsoftGroup = { name: "🪟 Microsoft", type: "select", proxies: [SELECT_NAME, AUTO_NAME, ...regionNames], icon: "" };
  const appleGroup = { name: "🍎 Apple", type: "select", proxies: [SELECT_NAME, AUTO_NAME, ...regionNames], icon: "" };
  const steamGroup = { name: "🎮 Steam", type: "select", proxies: [SELECT_NAME, AUTO_NAME, ...regionNames], icon: "" };
  const tiktokGroup = { name: "📱 TikTok", type: "select", proxies: [SELECT_NAME, AUTO_NAME, ...regionNames], icon: "" };
  const twitterGroup = { name: "🐦 Twitter", type: "select", proxies: [SELECT_NAME, AUTO_NAME, ...regionNames], icon: "" };
  ...
  config["proxy-groups"] = [selectGroup, autoGroup, lbGroup, adBlockGroup, privateGroup, domesticGroup, aiGroup, mediaGroup, youtubeGroup, googleGroup, telegramGroup, microsoftGroup, appleGroup, steamGroup, tiktokGroup, twitterGroup, spotifyGroup, globalServiceGroup, fallbackGroup, remoteToolGroup, ...regionGroups];
  ...
  config["rule-providers"] = {
    private: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/private.mrs`, path: "./ruleset/private.mrs" },
    private_ip: { ...ruleProviderCommonIpcidr, url: `${BASE_META}/geoip/private.mrs`, path: "./ruleset/private_ip.mrs" },
    cn: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/cn.mrs`, path: "./ruleset/cn.mrs" },
    "geolocation-cn": { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/geolocation-cn.mrs`, path: "./ruleset/geolocation-cn.mrs" },
    games_cn: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/category-games@cn.mrs`, path: "./ruleset/games_cn.mrs" },
    apple_cn: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/apple@cn.mrs`, path: "./ruleset/apple_cn.mrs" },
    microsoft_cn: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/microsoft@cn.mrs`, path: "./ruleset/microsoft_cn.mrs" },
    ads: { ...ruleProviderCommonDomain, url: "https://cdn.jsdelivr.net/gh/217heidai/adblockfilters@main/rules/adblockmihomolite.mrs", path: "./ruleset/ads.mrs" },
    ai: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/category-ai-!cn.mrs`, path: "./ruleset/ai.mrs" },
    youtube: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/youtube.mrs`, path: "./ruleset/youtube.mrs" },
    google: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/google.mrs`, path: "./ruleset/google.mrs" },
    google_ip: { ...ruleProviderCommonIpcidr, url: `${BASE_META}/geoip/google.mrs`, path: "./ruleset/google_ip.mrs" },
    telegram: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/telegram.mrs`, path: "./ruleset/telegram.mrs" },
    telegram_ip: { ...ruleProviderCommonIpcidr, url: `${BASE_META}/geoip/telegram.mrs`, path: "./ruleset/telegram_ip.mrs" },
    microsoft: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/microsoft.mrs`, path: "./ruleset/microsoft.mrs" },
    github: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/github.mrs`, path: "./ruleset/github.mrs" },
    apple: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/apple.mrs`, path: "./ruleset/apple.mrs" },
    steam: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/steam.mrs`, path: "./ruleset/steam.mrs" },
    tiktok: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/tiktok.mrs`, path: "./ruleset/tiktok.mrs" },
    twitter: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/twitter.mrs`, path: "./ruleset/twitter.mrs" },
    twitter_ip: { ...ruleProviderCommonIpcidr, url: `${BASE_META}/geoip/twitter.mrs`, path: "./ruleset/twitter_ip.mrs" },
    spotify: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/spotify.mrs`, path: "./ruleset/spotify.mrs" },
    netflix: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/netflix.mrs`, path: "./ruleset/netflix.mrs" },
    netflix_ip: { ...ruleProviderCommonIpcidr, url: `${BASE_META}/geoip/netflix.mrs`, path: "./ruleset/netflix_ip.mrs" },
    disney: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/disney.mrs`, path: "./ruleset/disney.mrs" },
    hbo: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/hbo.mrs`, path: "./ruleset/hbo.mrs" },
    twitch: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/twitch.mrs`, path: "./ruleset/twitch.mrs" },
    gfw: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/gfw.mrs`, path: "./ruleset/gfw.mrs" },
    wechat: { ...ruleProviderClassical, url: `${BASE_BLACK}/WeChat/WeChat.yaml`, path: "./ruleset/wechat.yaml" },
    phishing: { ...ruleProviderTextDomain, url: "https://ruleset.skk.moe/Clash/domainset/reject_phishing.txt", path: "./ruleset/phishing.txt" },
    icloud: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/icloud.mrs`, path: "./ruleset/icloud.mrs" },
    gitlab: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/gitlab.mrs`, path: "./ruleset/gitlab.mrs" },
    facebook: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/facebook.mrs`, path: "./ruleset/facebook.mrs" },
    instagram: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/instagram.mrs`, path: "./ruleset/instagram.mrs" },
    linkedin: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/linkedin.mrs`, path: "./ruleset/linkedin.mrs" },
    discord: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/discord.mrs`, path: "./ruleset/discord.mrs" },
    epicgames: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/epicgames.mrs`, path: "./ruleset/epicgames.mrs" },
    ea: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/ea.mrs`, path: "./ruleset/ea.mrs" },
    ubisoft: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/ubisoft.mrs`, path: "./ruleset/ubisoft.mrs" },
    blizzard: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/blizzard.mrs`, path: "./ruleset/blizzard.mrs" },
    paypal: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/paypal.mrs`, path: "./ruleset/paypal.mrs" },
    aws: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/aws.mrs`, path: "./ruleset/aws.mrs" },
    azure: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/azure.mrs`, path: "./ruleset/azure.mrs" },
    dropbox: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/dropbox.mrs`, path: "./ruleset/dropbox.mrs" },
    onedrive: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/onedrive.mrs`, path: "./ruleset/onedrive.mrs" },
    scholar: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/category-scholar-!cn.mrs`, path: "./ruleset/scholar.mrs" },
    hulu: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/hulu.mrs`, path: "./ruleset/hulu.mrs" },
    amazon: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/amazon.mrs`, path: "./ruleset/amazon.mrs" },
    bahamut: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/bahamut.mrs`, path: "./ruleset/bahamut.mrs" },
    biliintl: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/biliintl.mrs`, path: "./ruleset/biliintl.mrs" },
    abema: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/abema.mrs`, path: "./ruleset/abema.mrs" },
    bbc: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/bbc.mrs`, path: "./ruleset/bbc.mrs" },
    cloudflare_ip: { ...ruleProviderCommonIpcidr, url: `${BASE_META}/geoip/cloudflare.mrs`, path: "./ruleset/cloudflare_ip.mrs" },
    fastly_ip: { ...ruleProviderCommonIpcidr, url: `${BASE_META}/geoip/fastly.mrs`, path: "./ruleset/fastly_ip.mrs" },
    tracker: { ...ruleProviderCommonDomain, url: `${BASE_META}/geosite/tracker.mrs`, path: "./ruleset/tracker.mrs" }
  };
