...
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"; SETTINGS="$SCRIPT_DIR/settings.conf"; GENERATE="$SCRIPT_DIR/generate.sh"; RULES="$SCRIPT_DIR/rulesets.local.conf"; UNINSTALL="$SCRIPT_DIR/uninstall.sh"; UPDATE="$SCRIPT_DIR/update.sh"; SETTINGS_LOADER="$SCRIPT_DIR/tools/load-settings.sh"
...
[[ -x "$UPDATE" ]] || err "缺少安全更新器 update.sh"
...
for line in lines:
    if re.match(r'^AIRPORT_SUB_URL=', line): out.append('AIRPORT_SUB_URL=' + shlex.quote(new)); found=True
    else: out.append(line)
if not found: out.append('AIRPORT_SUB_URL=' + shlex.quote(new))
...
rules_menu(){ local c; echo "1) 查看当前覆盖"; echo "2) 增加/替换规则集"; echo "3) 禁用规则集"; echo "4) 恢复模板默认"; echo "5) 返回"; read -r -p "选择: " c; case "$c" in 1) list_rules;;2) add_rule;;3) disable_rule;;4) restore_rule;;5) return;;*) warn "无效选择";;esac; }
...
menu(){ local c; while :; do echo; echo -e "${CYAN}Mihomo-Full 管理${NC}"; echo "1) 更换机场订阅（自动重新生成）"; echo "2) 更新 VPS 落地节点"; echo "3) 规则集管理（增加/替换/禁用/恢复）"; echo "4) 查看当前状态"; echo "5) 配置完整性审计"; echo "6) 更新 Mihomo Full"; echo "7) 卸载 Mihomo Full（不会删除 v2ray-agent）"; echo "8) 退出"; read -r -p "请选择 [1-8]: " c; case "$c" in 1) change_airport;;2) bash "$GENERATE";;3) rules_menu;;4) show_status;;5) "$0" --audit;;6) "$0" --update;;7) uninstall_menu; return;;8) return;;*) warn "无效选择";;esac; done; }
case "${1:-menu}" in airport|change|1) change_airport;; generate) bash "$GENERATE";; update|2) exec "$UPDATE";; rules|ruleset|3) rules_menu;; status|4) show_status;; audit|5) "$0" --audit;; uninstall|7) uninstall_menu;; menu|*) menu;; esac