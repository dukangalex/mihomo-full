...
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"; SETTINGS="$SCRIPT_DIR/settings.conf"; GENERATE="$SCRIPT_DIR/generate.sh"; RULES="$SCRIPT_DIR/rulesets.local.conf"; UNINSTALL="$SCRIPT_DIR/uninstall.sh"
RED='\033[0;31m'; GREEN='\033[0;32m'; YELLOW='\033[1;33m'; CYAN='\033[0;36m'; NC='\033[0m'
err(){ echo -e "${RED}[✗]${NC} $1" >&2; exit 1; }; info(){ echo -e "${GREEN}[+]${NC} $1"; }; warn(){ echo -e "${YELLOW}[!]${NC} $1"; }
[[ $EUID -eq 0 ]] || err "请使用 root 运行"; [[ -f "$SETTINGS" ]] || err "找不到 $SETTINGS"; [[ -x "$GENERATE" ]] || err "找不到可执行的 generate.sh"; [[ -f "$RULES" ]] || { printf '%s\n' '# provider|https_mrs_url|behavior|target|enabled' > "$RULES"; chmod 600 "$RULES"; }
get_current_url(){ sed -n 's/^AIRPORT_SUB_URL=//p' "$SETTINGS" | head -n1 | sed -E "s/^['\"]//; s/['\"]$//"; }
...
set_airport_url(){ local new="$1" tmp; valid_url "$new" || err "订阅地址必须是 HTTPS URL，且不能含空格、引号或换行"; tmp="${SETTINGS}.tmp"; awk -v u="$new" 'BEGIN{found=0} /^AIRPORT_SUB_URL=/{print "AIRPORT_SUB_URL=\"" u "\""; found=1; next} {print} END{if(!found) print "AIRPORT_SUB_URL=\"" u "\""}' "$SETTINGS" > "$tmp"; chmod 600 "$tmp"; mv "$tmp" "$SETTINGS"; }
noninteractive(){ local action="${1:-}"; shift || true; case "$action" in
  ...
  --check) test -s "$SETTINGS" && test -s "$SCRIPT_DIR/template.yaml" && test -s "$GENERATE" && test -s "$UNINSTALL" && bash -n "$GENERATE" && bash -n "$0" && bash -n "$UNINSTALL"; info "管理、生成、卸载脚本语法检查通过";;
...
show_status(){ echo; echo -e "${CYAN}Mihomo-Full 状态${NC}"; echo "-------------------------"; echo "机场：$(mask_url "$(get_current_url)")"; echo "配置：${SCRIPT_DIR}/output/full-config.yaml"; echo "完整订阅路径：$(sed -n 's/^FIXED_FULL_CONFIG_PATH=//p' "$SETTINGS" | head -n1)"; echo "落地节点路径：$(sed -n 's/^FIXED_EXIT_NODES_PATH=//p' "$SETTINGS" | head -n1)"; echo; }
